...
using pair = std::pair<std::string, int>;   // String: word, int: frequency
...
bool sortBy(const pair& t1, const pair& t2) {
    ...
    if (t1.second == t2.second)         // If same frequency, alphabetical order 
    {
        return(t1.first < t2.first);    // sort alphabetically by word
    }
    return (t1.second > t2.second);     // Sort by frequency
}
...
int main() {
    ...
    while (in_File >> Word) {   // As long as the stream can take in words
        ...
        removeChar(Word);   // Remove special characters
        lowerCase(Word);    // Change all to lowercase
    ...
    // push_back does not exist for map
